...
public class GroundCell : MonoBehaviour {
	...
	// The main game mechanic. Each grid cell has this script attached. Instantiates buildings, supervises their construction procedure and calculates the income.
	...
	void Start () {
		...
		if (PlayerPrefs.HasKey("CreationTimestamp_" + x_coord.ToString() + "_" + y_coord.ToString()) ){
			if (PlayerPrefs.HasKey("CompletionTimestamp_" + x_coord.ToString() + "_" + y_coord.ToString())){	
				...
				built = true;
				building.GetComponent<BuiltBuilding>().Built = true;
	...
	void InstantiateBuilding(GlobalManager.BuildingType bt){
		...
		switch (bt)
		{
			...
			case GlobalManager.BuildingType.Hotel:
				...
				break;
			case GlobalManager.BuildingType.Factory:
				objToBuild = gm_Instance.factoryBuildingObj;
				incomeAmountPerSec = 20;
				buildingTypeString = "Factory";
				buildingTypeCost = 1000;
				break;
			default:
				break;
		}
